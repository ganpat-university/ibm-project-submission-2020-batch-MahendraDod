...
def fetch_data_from_database(keyword):
    ...
    query = f"SELECT * FROM user_feedback WHERE feed_name LIKE '%{keyword}%'"
...
def chat(user_input):
    ...
    elif user_input.lower()=="what is resume analyzer":
        response= "Resume Analyzer which parses information from a resume using natural language processing and finds the keywords, cluster them onto sectors based on their keywords. And lastly show recommendations, predictions, analytics to the applicant / recruiter based on keyword matching."
    elif user_input.lower()=="about project":
        response = "A tool which parses information from a resume using natural language processing and finds the keywords, cluster them onto sectors based on their keywords. And lastly show recommendations, predictions, analytics to the applicant / recruiter based on keyword matching."
    elif user_input.lower()=="technology used":
        response = "Backend: Streamlit And Python, Database: MySQL"
    else:
        response = "Cant Undertand."  # Default response
    keywords = extract_keywords(user_input)
    for keyword in keywords:
        ...
        if data_from_db:
            ...
            table += "<tr><th>ID</th><th>Name</th><th>Feedback</th></tr>"
